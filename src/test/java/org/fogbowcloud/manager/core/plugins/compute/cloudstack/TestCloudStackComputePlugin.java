...
public class TestCloudStackComputePlugin {
	...
	private static final BasicStatusLine BASIC_STATUS_LINE = new BasicStatusLine(PROTO, 200, "test reason");
	
	private static void recordHTTPClientWrapperRequest(HttpClientWrapper httpClient,
			Token token, String requestType, String url, String response) {
		...
		HttpResponseWrapper returned = new HttpResponseWrapper(BASIC_STATUS_LINE, response);
	...
	private static String createURL(String command, String... queryParts) {
		URIBuilder uriBuilder = null;
		try {
	...
	public void testRequestInstace() {
		...
		String deployyVMUrl = createURL(CloudStackComputePlugin.DEPLOY_VM_COMMAND, 
				CloudStackComputePlugin.TEMPLATE_ID, imageId,
				CloudStackComputePlugin.ZONE_ID, ZONE_ID,
				SERVICE_OFFERING_PARAMETER, "62d5f174-2f1e-42f0-931e-07600a05470e");
		recordHTTPClientWrapperRequest(httpClient, token, POST, 
				deployyVMUrl, RESPONSE_DEPLOY_VM);
		
		String getVMUrl = createURL(CloudStackComputePlugin.LIST_SERVICE_OFFERINGS_COMMAND);
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				getVMUrl, RESPONSE_GET_FLAVOR);
		
		CloudStackComputePlugin computePlugin = createPlugin(httpClient, extraProperties);
		computePlugin.requestInstance(token, categories, new HashMap<String, String>(), imageId);
	}
	...
	public void testRequestInstanceNullImageId() {
		...
		computePlugin.requestInstance(token, categories, new HashMap<String, String>(), null);
	}
	...
	public void testRequestInstanceZoneIdNull() {
		...
		Token token = new Token("api:key", null, new Date(), null);
		CloudStackComputePlugin computePlugin = createPlugin(null, null);
		computePlugin.requestInstance(token, categories, new HashMap<String, String>(), "");
	}
	...
	public void testRequestInstanceWithBadResponse() {
		...
		String deployyVMUrl = createURL(CloudStackComputePlugin.DEPLOY_VM_COMMAND, 
				...
				CloudStackComputePlugin.USERDATA, "userdata");
		recordHTTPClientWrapperRequest(httpClient, token, POST, 
				deployyVMUrl, BAD_RESPONSE_STRING);
		String getVMUrl = createURL(CloudStackComputePlugin.LIST_SERVICE_OFFERINGS_COMMAND);
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				getVMUrl, RESPONSE_GET_FLAVOR);
	...
	public void testGetInstancesOneInstance() {
		...
		String deployyVMUrl = createURL(CloudStackComputePlugin.LIST_VMS_COMMAND);
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				deployyVMUrl, RESPONSE_ONE_INSTANCE);
	...
	public void testGetInstancesMultipleInstances() {
		...
		String deployyVMUrl = createURL(CloudStackComputePlugin.LIST_VMS_COMMAND);
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				deployyVMUrl, RESPONSE_MULTIPLE_INSTANCES);
	...
	public void testGetInstancesNoInstance() {
		...
		String deployyVMUrl = createURL(CloudStackComputePlugin.LIST_VMS_COMMAND);
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				deployyVMUrl, RESPONSE_NO_INSTANCE);
	...
	public void testRemoveInstance() {
		...
		HttpClientWrapper httpClient = Mockito.mock(HttpClientWrapper.class);
		String getInstancesURL = createURL(CloudStackComputePlugin.LIST_VMS_COMMAND); 
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				getInstancesURL, RESPONSE_ONE_INSTANCE);
	...
	public void testRemoveInstanceWithoutInstance() {
		...
		HttpClientWrapper httpClient = Mockito.mock(HttpClientWrapper.class);
		String getInstancesURL = 
				createURL(CloudStackComputePlugin.LIST_VMS_COMMAND); 
		recordHTTPClientWrapperRequest(httpClient, token, GET, 
				getInstancesURL, RESPONSE_NO_INSTANCE);
		
		CloudStackComputePlugin cscp = createPlugin(httpClient, null);
		cscp.removeInstances(token);
		Mockito.verify(httpClient, Mockito.never()).doPost(uriBuilder.toString());
	}
	...
	public void testGetResourceInfoOneInstance() {
		...
		String urlListVM = 
				createURL(CloudStackComputePlugin.LIST_VMS_COMMAND);
		String urlListResources = 
				createURL(CloudStackComputePlugin.LIST_RESOURCE_LIMITS_COMMAND);
		recordHTTPClientWrapperRequest
		(httpClient, token, GET, urlListVM, RESPONSE_ONE_INSTANCE);
		recordHTTPClientWrapperRequest
		(httpClient, token, GET, urlListResources, RESPONSE_RESOURCES_INFO);
	...
	public void testGetResourcesInfoMultipleInstances() {
		...
		String urlListResources = 
				createURL(CloudStackComputePlugin.LIST_RESOURCE_LIMITS_COMMAND);
		recordHTTPClientWrapperRequest
		(httpClient, token, GET, urlListVM, RESPONSE_MULTIPLE_INSTANCES);
		recordHTTPClientWrapperRequest
		(httpClient, token, GET, urlListResources, RESPONSE_RESOURCES_INFO);
	...
	public void testGetResourcesNoInstances() {
		...
		String urlListResources = 
				createURL(CloudStackComputePlugin.LIST_RESOURCE_LIMITS_COMMAND);
		recordHTTPClientWrapperRequest
		(httpClient, token, GET, urlListVM, RESPONSE_NO_INSTANCE);
		recordHTTPClientWrapperRequest
		(httpClient, token, GET, urlListResources, RESPONSE_RESOURCES_INFO);
	...
	private static final String IMAGE_URL = VALID_IMAGE_PATH.replace(IMAGE_DOWNLOADED_BASE_PATH, IMAGE_DOWNLOADED_BASE_URL + "/");
	
	@Test
	public void testUploadImage() {
		Token token = new Token("api:key", null, null, null);
		String registerTemplateURL = createURL(CloudStackComputePlugin.REGISTER_TEMPLATE_COMMAND, 
		...
		HttpClientWrapper httpClient = Mockito.mock(HttpClientWrapper.class);
		recordHTTPClientWrapperRequest(httpClient, token, POST, registerTemplateURL, "response");
	...
	public void testUploadImageWithoutOSType() {
		...
		String registerTemplateURL = createURL(CloudStackComputePlugin.REGISTER_TEMPLATE_COMMAND, 
                ...
                CloudStackComputePlugin.URL, IMAGE_URL,
                CloudStackComputePlugin.IS_PUBLIC, Boolean.TRUE.toString());
		String listOSTypeURL = createURL(CloudStackComputePlugin.LIST_OS_TYPES_COMMAND);
        HttpClientWrapper httpClient = Mockito.mock(HttpClientWrapper.class);
        recordHTTPClientWrapperRequest(httpClient, token, POST, registerTemplateURL, "response");
        recordHTTPClientWrapperRequest(httpClient, token, GET, listOSTypeURL, RESPONSE_OS_TYPE);
	...
	public void testGetImageId() {
		Token token = new Token("api:key", null, null, null);
		String listTemplatesURL = createURL(CloudStackComputePlugin.LIST_TEMPLATES_COMMAND,
				CloudStackComputePlugin.TEMPLATE_FILTER, TEMPLATE_FILTER_EXECUTABLE);
		HttpClientWrapper httpClient = Mockito.mock(HttpClientWrapper.class);
		recordHTTPClientWrapperRequest(httpClient, token, GET, listTemplatesURL, RESPONSE_LIST_TEMPLATES);
		CloudStackComputePlugin computePlugin = createPlugin(httpClient, null);
	...
	public void testGetImageStateActive() {
		...
		CloudStackComputePlugin computePlugin = createPlugin(httpClient, null);
		ImageState imageState = computePlugin.getImageState(token, VALID_IMAGE_NAME);
	...
	public void testGetImageStatePending() {
		...
		CloudStackComputePlugin computePlugin = createPlugin(httpClient, null);
		ImageState imageState = computePlugin.getImageState(token, PENDING_IMAGE_NAME);
	...
	public void testGetImageStateBadRequest() {
		...
		CloudStackComputePlugin computePlugin = createPlugin(httpClient, null);
		ImageState imageState = computePlugin.getImageState(token, INVALID_IMAGE_NAME);
